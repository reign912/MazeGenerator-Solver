...
def ellipse(x, y, width, height):
    ...
    if shape.quadric is None:
        shape.quadric = gl.gluNewQuadric()
    if shape.ellipseFillDL is None:
    ...
    if attrib.fillColor is not None:
    ...
    if attrib.strokeColor is not None:
...
def line(*coords):
    ...
    if n == 2:
        ...
        p1, p2 = coords[:int(round(n / 2))], coords[int(round(n / 2)):]
...
def box(*args):
    ...
    if attrib.fillColor is not None:
        ...
        if shape.cubeFillVL is None: shape.cubeFillVL = cubeFillList()
